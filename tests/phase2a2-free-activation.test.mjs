...
test('new Free organizations get a dashboard activation path', () => {
  ...
  assert.match(usage, /AT TIME ZONE 'UTC'/)
  assert.match(dashboardClient, /showActivationPanel = totalEventCount === 0/)
  assert.match(dashboardClient, /isFreePlan = tenant\.plan === 'free'/)
  assert.match(dashboardClient, /\{isFreePlan \? 'Finish Free setup' : 'Finish setup'\}/)
  assert.match(dashboardClient, /Create a source to start receiving webhooks\./)
  ...
  assert.match(eventsRoute, /monthlyEventLimit/)
  assert.match(eventsRoute, /plan: access\.tenant\.plan/)
...
test('platform admin owned workspaces get internal Growth entitlements', () => {
  ...
  assert.match(entitlements, /changeOrganizationPlan/)
  assert.match(entitlements, /platform_admin_effective_plan/)
