...
test('sidebar palette control is a dropdown with broad style names', () => {
  ...
  assert.match(palette, /\/api\/account\/preferences/)
  assert.match(palette, /normalizeAppPalette/)
  assert.match(read('src/lib/app-palette.ts'), /value === 'signal' \|\| value === 'monitoring'/)
  assert.match(read('src/app/api/account/preferences/route.ts'), /auth\.api\.getSession/)
  assert.match(read('src/app/api/account/preferences/route.ts'), /appPalette/)
